...
router.get('/:id', (req, res) => {
  const p = req.db.get('SELECT * FROM payments WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
  if (!p) return res.json({ errorcode: 404, errormsg: 'Payment not found' });
  res.json(p);
});

router.get('/unpaid/:customer_id', (req, res) => {
  const invoices = req.db.all("SELECT id, invoice_number, date, due_date, total, amount_paid, amount_due FROM invoices WHERE customer_id = ?" + (req.isSuperadmin ? '' : " AND company_id = ?") + " AND status != 'paid' AND status != 'draft' ORDER BY date DESC", req.isSuperadmin ? [req.params.customer_id] : [req.params.customer_id, req.companyId]);
  res.json({ invoices });
});
...
router.post('/', (req, res) => {
  ...
  const id = uuidv4();
  req.db.run('INSERT INTO payments (id, user_id, company_id, invoice_id, customer_id, customer_name, payment_number, date, method, reference, amount, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
    [id, req.userId, req.companyId, invoice_id||null, customer_id||null, customer_name||'', pn, date||new Date().toISOString().split('T')[0], method||'', reference||'', amount, notes||'']);
  if (invoice_id) {
    const inv = req.db.get('SELECT total, amount_paid FROM invoices WHERE id = ? AND company_id = ?', [invoice_id, req.companyId]);
    if (inv) {
...
router.put('/:id', (req, res) => {
  const existing = req.db.get('SELECT * FROM payments WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
  ...
  if (existing.invoice_id) {
    ...
    if (diff !== 0) {
      const inv = req.db.get('SELECT total, amount_paid FROM invoices WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [existing.invoice_id] : [existing.invoice_id, req.companyId]);
      if (inv) {
...
router.delete('/:id', (req, res) => {
  const p = req.db.get('SELECT * FROM payments WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
  if (p && p.invoice_id) {
    const inv = req.db.get('SELECT total, amount_paid FROM invoices WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [p.invoice_id] : [p.invoice_id, req.companyId]);
    if (inv) {
  ...
  req.db.run('DELETE FROM payments WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
  res.json({ success: true });
});
