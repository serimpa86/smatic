...
router.post('/', (req, res) => {
  ...
  const id = uuidv4();
  req.db.run('INSERT INTO items (id, user_id, code, name, description, price, cost_price, tax_rate, tax_name, unit, stock_quantity, stock_warning_level, track_inventory, active) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
    [id, req.userId, code||'', name, description||'', price||0, cost_price||0, tax_rate||0, tax_name||'', unit||'', stock_quantity||0, stock_warning_level||0, track_inventory||0, active!==undefined?active:1]);
  res.json({ id });
  generateQrAsync(req, id, code || name);
});

router.put('/:id', (req, res) => {
  const existing = req.db.get('SELECT id, code, qr_code FROM items WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  if (!existing) return res.json({ errorcode: 404, errormsg: 'Producto no encontrado' });
  const { code, name, description, price, cost_price, tax_rate, tax_name, unit, stock_quantity, stock_warning_level, track_inventory, active } = req.body;
  req.db.run("UPDATE items SET code=?, name=?, description=?, price=?, cost_price=?, tax_rate=?, tax_name=?, unit=?, stock_quantity=?, stock_warning_level=?, track_inventory=?, active=?, updated_at=datetime('now') WHERE id=?" + (req.isSuperadmin ? '' : ' AND user_id=?'),
    req.isSuperadmin
      ...
      : [code||'', name, description||'', price||0, cost_price||0, tax_rate||0, tax_name||'', unit||'', stock_quantity||0, stock_warning_level||0, track_inventory||0, active!==undefined?active:1, req.params.id, req.userId]);
...
router.post('/:id/image', upload.single('image'), (req, res) => {
  const existing = req.db.get('SELECT id, image_url FROM items WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  if (!existing) return res.json({ errorcode: 404, errormsg: 'Producto no encontrado' });
...
router.delete('/:id/qrcode', (req, res) => {
  const existing = req.db.get('SELECT id, qr_code FROM items WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  if (!existing) return res.json({ errorcode: 404, errormsg: 'Producto no encontrado' });
  if (existing.qr_code) {
...
router.delete('/:id', (req, res) => {
  const item = req.db.get('SELECT id, image_url, qr_code FROM items WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  if (item) {
    if (item.image_url) {
  ...
  req.db.run('DELETE FROM items WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  res.json({ success: true });
});
