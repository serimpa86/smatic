...
router.get('/dashboard', (req, res) => {
  ...
  const userFilter = req.isSuperadmin ? '' : 'user_id = ? AND';
  const params = req.isSuperadmin ? [] : [req.userId];
  ...
  const totalCustomers = req.db.get('SELECT COUNT(*) as c FROM customers' + (req.isSuperadmin ? '' : ' WHERE user_id = ?'), req.isSuperadmin ? [] : [req.userId]);
  const recentInvoices = req.db.all("SELECT invoice_number, customer_name, total, status, date, id FROM invoices WHERE " + (req.isSuperadmin ? "1=1" : "user_id = ?") + " ORDER BY created_at DESC LIMIT 10", req.isSuperadmin ? [] : [req.userId]);
  const monthlyStats = req.db.all("SELECT strftime('%Y-%m', date) as month, COUNT(*) as count, COALESCE(SUM(total),0) as total FROM invoices WHERE " + (req.isSuperadmin ? "" : "user_id = ? AND ") + "status != 'draft' AND status != 'cancelled' GROUP BY month ORDER BY month DESC LIMIT 12", req.isSuperadmin ? [] : [req.userId]);
  const pendingFilter = req.isSuperadmin ? '' : 'user_id = ? AND';
  const pendingParams = req.isSuperadmin ? [] : [req.userId];
...
router.get('/sales', (req, res) => {
  const { start, end } = req.query;
  let sql = "SELECT i.*, c.name as customer_name FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id WHERE " + (req.isSuperadmin ? "1=1" : "i.user_id = ?") + " AND i.status != 'draft' AND i.status != 'cancelled'";
...
router.get('/taxes', (req, res) => {
  const { start, end } = req.query;
  let sql = "SELECT ii.tax_name, ii.tax_rate, SUM(ii.total) as taxable_amount FROM invoice_items ii JOIN invoices i ON ii.invoice_id = i.id WHERE " + (req.isSuperadmin ? "1=1" : "i.user_id = ?");
  let params = req.isSuperadmin ? [] : [req.userId];
  if (start && end) { sql += ' AND i.date >= ? AND i.date <= ?'; params.push(start, end); }
