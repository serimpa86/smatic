...
router.get('/', (req, res) => {
  ...
  let where = 'WHERE 1=1';
  let params = [];
  if (!req.isSuperadmin) { where += ' AND r.user_id = ?'; params.push(req.userId); }
...
router.get('/:id', (req, res) => {
  const r = req.db.get('SELECT * FROM refunds WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
...
router.post('/', (req, res) => {
  ...
  const count = req.db.get('SELECT COUNT(*) as c FROM refunds WHERE user_id = ?', [req.userId]);
  const num = 'REF-' + String((count.c || 0) + 1001);
  const id = uuidv4();
  req.db.run(`INSERT INTO refunds (id, user_id, payment_id, customer_id, customer_name, refund_number, date, amount, reason) VALUES (?,?,?,?,?,?,?,?,?)`,
    [id, req.userId, payment_id||null, customer_id||null, customer_name||'', num, date||new Date().toISOString().split('T')[0], amount||0, reason||'']);
  res.json({ id, refund_number: num });
});

router.put('/:id', (req, res) => {
  const existing = req.db.get('SELECT id FROM refunds WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
...
router.delete('/:id', (req, res) => {
  req.db.run('DELETE FROM refunds WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  res.json({ success: true });
});
