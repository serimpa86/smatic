...
router.get('/:id', (req, res) => {
  const note = req.db.get('SELECT * FROM credit_notes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
...
router.post('/', (req, res) => {
  ...
  const settings = req.db.get('SELECT credit_note_prefix, next_credit_note_number FROM business_settings WHERE company_id = ?', [req.companyId]);
  ...
  total = subtotal;
  req.db.run(`INSERT INTO credit_notes (id, user_id, company_id, credit_note_number, invoice_id, customer_id, customer_name, customer_email, date, status, currency, currency_symbol, subtotal, tax_total, total, reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [id, req.userId, req.companyId, num, invoice_id||null, customer_id||null, customer_name||'', customer_email||'', date||new Date().toISOString().split('T')[0], 'open', currency||'USD', currency_symbol||'$', subtotal, 0, total, reason||'']);
  if (items && items.length) {
  ...
  req.db.run('UPDATE business_settings SET next_credit_note_number = next_credit_note_number + 1 WHERE company_id = ?', [req.companyId]);
  res.json({ id, credit_note_number: num });
});

router.put('/:id', (req, res) => {
  const existing = req.db.get('SELECT id FROM credit_notes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
...
router.delete('/:id', (req, res) => {
  ...
  req.db.run('DELETE FROM credit_notes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.companyId]);
  res.json({ success: true });
});
...
router.post('/:id/apply', (req, res) => {
  ...
  const inv = req.db.get('SELECT total, amount_paid, amount_due FROM invoices WHERE id = ?' + (req.isSuperadmin ? '' : ' AND company_id = ?'), req.isSuperadmin ? [invoice_id] : [invoice_id, req.companyId]);
