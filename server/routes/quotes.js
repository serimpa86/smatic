...
router.get('/:id', (req, res) => {
  const q = req.db.get('SELECT * FROM quotes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
...
router.post('/', (req, res) => {
  ...
  const settings = req.db.get('SELECT quote_prefix, next_quote_number FROM business_settings WHERE user_id = ?', [req.userId]);
  const qNum = settings.quote_prefix + settings.next_quote_number;
  const id = uuidv4();
  req.db.run(`INSERT INTO quotes (id, user_id, quote_number, customer_id, customer_name, customer_email, customer_address, customer_shipping, date, expiry_date, payment_terms, po_number, salesperson, currency, currency_symbol, subtotal, discount_amount, discount_type, shipping_cost, tax_total, total, notes, private_notes, footer) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [id, req.userId, qNum, customer_id||null, customer_name||'', customer_email||'', customer_address||'', customer_shipping||'', date||new Date().toISOString().split('T')[0], expiry_date||'', payment_terms||'30', po_number||'', salesperson||'', currency||'USD', currency_symbol||'$', subtotal||0, discount_amount||0, discount_type||'percentage', shipping_cost||0, tax_total||0, total||0, notes||'', private_notes||'', footer||'']);
  ...
  req.db.run('UPDATE business_settings SET next_quote_number = next_quote_number + 1 WHERE user_id = ?', [req.userId]);
  res.json({ id, quote_number: qNum });
});

router.put('/:id', (req, res) => {
  const existing = req.db.get('SELECT id FROM quotes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  ...
  req.db.run("UPDATE quotes SET customer_id=?, customer_name=?, customer_email=?, customer_address=?, customer_shipping=?, date=?, expiry_date=?, payment_terms=?, po_number=?, salesperson=?, subtotal=?, discount_amount=?, discount_type=?, shipping_cost=?, tax_total=?, total=?, status=?, notes=?, updated_at=datetime('now') WHERE id=?" + (req.isSuperadmin ? '' : ' AND user_id=?'),
    req.isSuperadmin
      ...
      : [customer_id||null, customer_name||'', customer_email||'', customer_address||'', customer_shipping||'', date, expiry_date||'', payment_terms||'30', po_number||'', salesperson||'', subtotal||0, discount_amount||0, discount_type||'percentage', shipping_cost||0, tax_total||0, total||0, status||'draft', notes||'', req.params.id, req.userId]);
...
router.post('/:id/convert', (req, res) => {
  const quote = req.db.get('SELECT * FROM quotes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  ...
  req.db.run(`INSERT INTO invoices (id, user_id, invoice_number, customer_id, customer_name, customer_email, customer_address, customer_shipping, date, due_date, payment_terms, po_number, salesperson, currency, currency_symbol, subtotal, discount_amount, discount_type, shipping_cost, tax_total, total, amount_due, notes, footer, status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [invId, req.userId, invNum, quote.customer_id, quote.customer_name, quote.customer_email, quote.customer_address, quote.customer_shipping, new Date().toISOString().split('T')[0], '', quote.payment_terms, quote.po_number, quote.salesperson, quote.currency, quote.currency_symbol, quote.subtotal, quote.discount_amount, quote.discount_type, quote.shipping_cost, quote.tax_total, quote.total, quote.total, quote.notes, quote.footer, 'draft']);
...
router.delete('/:id', (req, res) => {
  req.db.run('DELETE FROM quotes WHERE id = ?' + (req.isSuperadmin ? '' : ' AND user_id = ?'), req.isSuperadmin ? [req.params.id] : [req.params.id, req.userId]);
  res.json({ success: true });
});
