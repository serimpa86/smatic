...
if ($Message -ne "") {
  ...
  Write-Host "[1/3] No commit message - skipping commit, pushing only..." -ForegroundColor Yellow
  & git push
  Write-Host "  Done" -ForegroundColor Green
}
...
ssh $SSH_USER@$SSH_HOST "cd $REMOTE_PATH; git pull origin main; npm install --production"
Write-Host "  Done" -ForegroundColor Green
